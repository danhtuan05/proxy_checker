...
async def check_proxies(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for proxy in proxies:
        if "📔 - Tổng số" in proxy:
            ...
            proxy_tach_sock = proxy.split('-')
            proxy_final_sock = proxy_tach_sock[2].strip()
            extracted.append(proxy_final_sock)
        elif "-" in proxy:
            proxy_tach = proxy.split('-')
            proxy_final = proxy_tach[1].strip()
            extracted.append(proxy_final)
        elif len(proxy.split(":"))==4:
            proxy_final = proxy.strip()
            extracted.append(proxy_final)
        else:
            pass
    result_proxy = "\n".join(extracted)
    ...
    if inactive_proxies:
        ...
        await update.message.reply_text("Kiểm tra lại Proxy không hoạt động !", reply_markup=reply_markup)
####################################################################################################
#CHECK VPS
    ...
    for proxy in proxies:
        if "Tổng" in proxy:
            ...
            try:
                proxy_tach = proxy.split('-')
                proxy_final = proxy_tach[1].strip()
                extracted.append(f'{proxy_final}')
            except ValueError:
                extracted.append(f'Chuỗi proxy không hợp lệ: {proxy}')
...
async def recheck_proxies(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if still_inactive_proxies:
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.callback_query.message.reply_text("Kiểm tra lại Proxy không hoạt động !", reply_markup=reply_markup)
##########################################################
#CHECK LẠI VPS
